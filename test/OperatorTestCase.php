...
class OperatorTestCase extends \PHPUnit\Framework\TestCase {
	...
	protected function assertHotColdConsumersSeeOrderedValues(array $expected, InteractiveProducer $producer) {
		$results = wait(
			all([
		...
		$this->assertEquals($expected, array_merge($results[1], $results[2]));
	}
